...
class Ui_Dialog(object):
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "<html><head/><body><p>Enter name for aperture...</p><p>If you want to base NRE on this star, begin the name with \'psf-star\' (without the quotes).</p><br/>A star/aperture so named will be the one used in estimating the instrumental psf when the<br/>generate NRE psf\' button is pressed.</p><p><br/>Of course, only a single aperture with a name starting<br/>with \'psf-star\' is allowed, but if you want other stars<br/>to use NRE, put the string \'psf-star\' somewhere in<br/>the name. (Usually the psf would be determined from the target star, but<br/>you can control this by appropriate naming.)</p><p><br/>If you want to suppress the re-centering of static masks within an<br/>aperture, put the string \'no-rc\' (or \'no_rc\' or \'no rc\') somewhere in the name. (Dynamic masks must<br/>relocate to do their job, so the presence of a \'no-rc\' will be ignored.)</p><p><br/>Apertures named \'empty\' or \'no-star\', or \'no_star\' or \'no star\' are<br/>treated as though they contain the \'no-rc\' string - this is because the<br/>N-brightest-pixel-mass-centroid re-centering of static circular masks,<br/>if allowed to \'hunt\' within the aperture, will find opportunistic<br/>clumps of pixels that are a little brighter than other clumps and thus<br/>generate a tiny, but false, signal - disabling recentering prevents this.))</p></body></html>"))
