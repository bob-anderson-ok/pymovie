...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.readAviFileButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>Note: avi files are read using OpenCV which employs ffmpeg to do the actual file reading. As a result, a wide range of avi codecs are standardly available.</p><p>In particular, avi files using Lagarith compression will be read without problem.</p><p>There is a remote possibility that an avi was recorded with an unusual codec and so fail to be read by PyMovie. In that case, you will need to use an external file conversion program that can decode the file as recorded and rewrite it using a more common codec.</p><p>.adv files must be Version 2, monochrome.</p></body></html>"))
        self.readAviFileButton.setText(_translate("MainWindow", "Open AVI/SER/ADV file"))
        self.createAVIWCSfolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>This button will only be active when a \'bare\' avi/ser/adv file is opened (rather than from an AVI/SER/ADV-WCS folder).</p><p>This button has been added because of the decision to perform timestamp OCR only on files that live in an AVI/SER/ADV-WCS folder. While that is a good way to collect the (many) files that result from OCR, WCS, PyMovie, and PyOTE operations, there are likely to be users that have opted for a flat (or flatter) directory structure. This button will ease (hopefully) the transition to an alternate file organization.</p><p>When clicked, a Directory dialog will appear to allow the user to select (or even create) the directory that is to hold the newly created (and filled) AVI/SER/ADV-WCS folder. It defaults to the directory that holds the opened video file.</p><p>The name of the newly created AVI/SER/ADV-WCS folder will always be the name of the video file with its outermost extension removed.</p><p>The folder will then be populated with an alias (Mac) to the avi or a Shortcut (Windows) to the avi, so this file structure will be frugal with file space.</p><p>Finally, behind-the-scenes, the program simulates the user clicking on <span style=\" font-weight:600; color:#fc0107;\">Select AVI/SER/ADV-WCS folder</span> and then clicking on <span style=\" font-weight:600; color:#fc0107;\">Accept</span> (possible because after a \'creation of AVI/SER/ADV-WCS folder\', the default directory/folder is set to that of the newly created folder). We could not imagine a workflow where a user would <span style=\" font-weight:600;\">not</span> want to immediately start processing the video from within an AVI/SER/ADV-WCS folder, so the automation seems reasonable and unlikely to get in anyone\'s way.<br/></p></body></html>"))
        self.createAVIWCSfolderButton.setText(_translate("MainWindow", "Create AVI/SER/ADV-WCS folder from file"))
        self.selectAviWcsFolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>An AVI/SER/ADV-WCS folder is a container for a single observation video and its related files. As such, there must be only a single .avi/.ser/.adv file in the folder either explicitly, or by shortcut (Windows) or alias (MacOS). This requirement is enforced.</p><p>This folder may also contain a simple text file that contains the Ra and DEC coordinates of the occulted star, filled in by the user through a dialog box that appears when this position information file is found missing during WCS calibration operations.</p><p>This folder is also where the WCS calibration data will be placed if the user opts for calibrating frame 0 by making an on-line submission to astrometry.net.</p><p>This folder is also the default location for storing csv files and copies of the last lightcurve plots.</p><p><span style=\" font-weight:600; color:#fc0107;\">Note:</span> avi files are read using OpenCV which employs ffmpeg to do the actual file reading. As a result, a wide range of avi codecs are standardly available.</p><p>In particular, avi files using Lagarith compression will be read without problem.</p><p>There is a remote possibility that an avi was recorded with an unusual codec and so fail to be read by PyMovie. In that case, you will need to use an external file conversion program that can decode the file as recorded and rewrite it using a more common codec.</p></body></html>"))
        self.selectAviWcsFolderButton.setText(_translate("MainWindow", "Select AVI/SER/ADV-WCS folder"))
        self.readFitsFolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>A <span style=\" color:#fc0107;\">FITS</span> folder should contain a set of *.fits files, all from a single recording.</p><p>There should be no other .fits files in that folder.</p><p>It is assumed that the alphabetic ordering of the file names corresponds to sequential frames.</p><p>There is a button titled <span style=\" font-weight:600; color:#fc0107;\">Show FITS/SER/ADV metadata</span> below that will read and display in the text box (lower left corner) all the metadata entries from the currently displayed .fits/.ser/.adv file image.</p></body></html>"))
        self.readFitsFolderButton.setText(_translate("MainWindow", "Select FITS folder"))
        self.metadataButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>Clicking this button will show (in the text box in the lower left corner of the GUI) all metadata from the currently displayed .fits/.ser/.adv file.</p><p>\'metadata\' is the text information that is present in the header of the .fits/.ser file or the SYSTEM and USER metadata that is present in an ADV Version 2 file.</p></body></html>"))
        self.metadataButton.setText(_translate("MainWindow", "Show FITS/SER/ADV file metadata"))
        self.enableAdvFrameStatusDisplay.setToolTip(_translate("MainWindow", "<html><head/><body><p>Data frames in ADV Version 2 files can have metadata associated with each frame. If this checkbox is checked, and there is metadata available, the metadata will be displayed in the text box (lower left corner of GUI) as each frame is read.</p></body></html>"))
        self.enableAdvFrameStatusDisplay.setText(_translate("MainWindow", "Display frame metadata for ADV frames"))
