...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.widget.setGeometry(QtCore.QRect(283, 6, 435, 195))
        ...
        self.backgroundSmoothingIntervalSpinBox.setMinimum(1)
        self.backgroundSmoothingIntervalSpinBox.setMaximum(1000)
        self.backgroundSmoothingIntervalSpinBox.setProperty("value", 1)
...
"aperture placement."))
        ...
        self.backgroundSmoothingLabel.setToolTip(_translate("MainWindow", "<html><head/><body><p>This value controls the window size (<span style=\" font-weight:700; color:#ff0000;\">n</span>) used for background smoothing.</p><p>PyMovie implements a simple sliding window algorithm to produce a \'smoothed\' per-pixel-background value using the following equation (after a \'startup\' of n frames):</p><p><span style=\" font-weight:700; color:#0000ff;\">avgBkgnd = (avgBkgnd * (n-1) + newBkgnd) / n</span></p><p>where <span style=\" font-weight:700; color:#0000ff;\">newBkgnd</span> is the background calculated for the current frame (per pixel).</p><p><span style=\" font-weight:700; color:#0000ff;\">avgBkgnd</span> is used in background-subtracted-signals to reduce the noise introduced by this needed/useful calculation. </p><p>That equation is: <span style=\" font-weight:700; color:#0000ff;\">signal = apertureSum - avgBkgnd * numPixelsInAperture</span></p><p>The default value for the window size upon startup is always 1 (not sticky between runs), but any value between 1 (no smoothing in this case) to 1000 may be used. The sliding window algorithm has a \'startup\' period equal to the window size during which a series of avgBkgnd values will be produced, each a little smoother than the last until n is reached. </p><p><span style=\" color:#ff0000;\">Do not expect this to have a significant effect.</span> The reason I say this is that the per-frame estimate of the per-pixel background level is based on (for the typical 51 x 51 aperture) a poisson fit to 1300 sample points from the background that have all star pixels excluded. The result is an <span style=\" font-weight:700; text-decoration: underline;\">already</span> very low noise background estimation - for example: improving it by a factor of 7 by using a smoothing count of 49 has shown a tiny effect only at low signal levels (near zero) and only during noise reduction extraction. Your mileage may vary.</p></body></html>"))
