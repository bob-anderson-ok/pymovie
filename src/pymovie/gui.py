...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.psfFrameCountLabel = QtWidgets.QLabel(self.layoutWidget1)
        self.psfFrameCountLabel.setMaximumSize(QtCore.QSize(16777215, 20))
        font = QtGui.QFont()
        font.setFamily(".SF NS Text")
        font.setPointSize(13)
        font.setBold(False)
        self.psfFrameCountLabel.setFont(font)
        self.psfFrameCountLabel.setObjectName("psfFrameCountLabel")
        self.horizontalLayout_34.addWidget(self.psfFrameCountLabel)
        self.psfFrameCountSpinBox = QtWidgets.QSpinBox(self.layoutWidget1)
        self.psfFrameCountSpinBox.setMaximumSize(QtCore.QSize(50, 16777215))
        font = QtGui.QFont()
        font.setFamily(".SF NS Text")
        font.setPointSize(13)
        font.setBold(True)
        self.psfFrameCountSpinBox.setFont(font)
        self.psfFrameCountSpinBox.setMinimum(2)
        self.psfFrameCountSpinBox.setMaximum(1000)
        self.psfFrameCountSpinBox.setProperty("value", 16)
        self.psfFrameCountSpinBox.setObjectName("psfFrameCountSpinBox")
        self.horizontalLayout_34.addWidget(self.psfFrameCountSpinBox)
        ...
        self.transportContinue = QtWidgets.QPushButton(self.layoutWidget2)
        self.transportContinue.setEnabled(False)
        self.transportContinue.setMinimumSize(QtCore.QSize(120, 0))
        self.transportContinue.setMaximumSize(QtCore.QSize(120, 16777215))
        font = QtGui.QFont()
        font.setFamily(".SF NS Text")
        font.setPointSize(13)
        font.setBold(True)
        self.transportContinue.setFont(font)
        self.transportContinue.setObjectName("transportContinue")
        self.horizontalLayout_19.addWidget(self.transportContinue)
        ...
        self.setLastFrameButton = QtWidgets.QPushButton(self.layoutWidget2)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.createAVIWCSfolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>This button will only be active when a \'bare\' avi/mov/ser/adv/aav/ravf file is opened (rather than from an AVI/MOV/SER/ADV/AAV-WCS/RAVF folder).</p><p>This button has been added because of the decision to perform timestamp OCR only on files that live in an AVI/MOV/SER/ADV/AAV-WCS/RAVF folder. While that is a good way to collect the (many) files that result from OCR, WCS, PyMovie, and PyOTE operations, there are likely to be users that have opted for a flat (or flatter) directory structure. This button will ease (hopefully) the transition to an alternate file organization.</p><p>When clicked, a Directory dialog will appear to allow the user to select (or even create) the directory that is to hold the newly created (and filled) AVI/MOV/SER/ADV/AAV-WCS/RAVF folder. It defaults to the directory that holds the opened video file.</p><p>The name of the newly created AVI/MOV/SER/ADV/AAV-WCS/RAVF folder will always be the name of the video file with its outermost extension removed.</p><p>The folder will then be populated with an alias (Mac) to the video or a Shortcut (Windows) to the video, so this file structure will be frugal with file space.</p><p>Finally, behind-the-scenes, the program simulates the user clicking on <span style=\" font-weight:600; color:#fc0107;\">Select AVI/MOV/SER/ADV/AAV-WCS/RAVF folder</span> and then clicking on <span style=\" font-weight:600; color:#fc0107;\">Accept</span> (possible because after a \'creation of AVI/MOV/SER/ADV/AAV-WCS/RAVF folder\', the default directory/folder is set to that of the newly created folder). We could not imagine a workflow where a user would <span style=\" font-weight:600;\">not</span> want to immediately start processing the video from within an AVI/MOV/SER/ADV/AAV-WCS/RAVF folder, so the automation seems reasonable and unlikely to get in anyone\'s way.<br/></p></body></html>"))
        self.createAVIWCSfolderButton.setText(_translate("MainWindow", "Create AVI/MOV/SER/ADV/AAV-WCS/RAVF folder from file"))
        self.selectAviWcsFolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>An AVI/MOV/SER/ADV/AAV-WCS/RAVF folder is a container for a single observation video and its related files. As such, there must be only a single .avi/.mov/.ser/.adv/.aav/.ravf file in the folder either explicitly, or by shortcut (Windows) or alias (MacOS). This requirement is enforced.</p><p>This folder may also contain a simple text file that contains the Ra and DEC coordinates of the occulted star, filled in by the user through a dialog box that appears when this position information file is found missing during WCS calibration operations.</p><p>This folder is also where the WCS calibration data will be placed if the user opts for calibrating frame 0 by making an on-line submission to astrometry.net.</p><p>This folder is also the default location for storing csv files and copies of the last lightcurve plots.</p><p><span style=\" font-weight:600; color:#fc0107;\">Note:</span> avi files are read using OpenCV which employs ffmpeg to do the actual file reading. As a result, a wide range of avi codecs are standardly available.</p><p>In particular, avi files using Lagarith compression will be read without problem.</p><p>There is a remote possibility that an avi was recorded with an unusual codec and so fail to be read by PyMovie. In that case, you will need to use an external file conversion program that can decode the file as recorded and rewrite it using a more common codec.</p><p>ADV and AAV files must be Version 2 monochrome.</p></body></html>"))
        self.selectAviWcsFolderButton.setText(_translate("MainWindow", "Select AVI/MOV/SER/ADV/AAV-WCS/RAVF folder"))
        self.readFitsFolderButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>A <span style=\" color:#fc0107;\">FITS</span> folder should contain a set of *.fits files, all from a single recording.</p><p>There should be no other .fits files in that folder.</p><p>It is assumed that the alphabetic ordering of the file names corresponds to sequential frames.</p><p>There is a button titled <span style=\" font-weight:600; color:#fc0107;\">Show FITS/SER/ADV/AAV/RAVF metadata</span> below that will read and display in the text box (lower left corner) all the metadata entries from the currently displayed .fits/.ser/.adv/.ravf file image.</p></body></html>"))
        self.readFitsFolderButton.setText(_translate("MainWindow", "Select FITS folder"))
        self.metadataButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>Clicking this button will show (in the text box in the lower left corner of the GUI) all metadata from the currently displayed .fits/.ser/.adv/.aav/.ravf file.</p><p>\'metadata\' is the text information that is present in the header of the .fits/.ser file or the SYSTEM and USER metadata that is present in an ADV, AAV Version 2 or RAVF files.</p></body></html>"))
        self.metadataButton.setText(_translate("MainWindow", "Show FITS/SER/ADV/AAV/RAVF file metadata"))
        self.enableAdvFrameStatusDisplay.setToolTip(_translate("MainWindow", "<html><head/><body><p>Data frames in ADV, AAV Version 2 and RAVF files can have metadata associated with each frame. If this checkbox is checked, and there is metadata available, the metadata will be displayed in the text box (lower left corner of GUI) as each frame is read.</p></body></html>"))
...
"aperture placement."))
        ...
        self.psfFrameCountLabel.setToolTip(_translate("MainWindow", "<html><head/><body><p>The number of frames that are used to determine the instrumental psf of a star is controlled by the number in this spin box.</p><p>The number needed is amazingly uncritical, so we default to a large enough number to sample well an integrated composite video. </p><p>The minimum is 2 and that number is also almost always enough. </p><p>You can change the number used if you have a special case that requires experimentation, but it is not sticky between runs.</p></body></html>"))
        self.psfFrameCountLabel.setText(_translate("MainWindow", "psf frame count:"))
        ...
        self.transportContinue.setToolTip(_translate("MainWindow", "<html><head/><body><p><span style=\" font-family:\'Arial\',\'sans-serif\';\">If you have paused the analysis run for some reason and wish to restart without calculating a new instrumental psf (only relevant if an aperture with a name starting with psf-star was present), click this button.<br/></span></p></body></html>"))
        self.transportContinue.setText(_translate("MainWindow", "continue"))
