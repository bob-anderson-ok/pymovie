...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.defRadiusSpinner = QtWidgets.QDoubleSpinBox(self.layoutWidget)
        ...
        self.maskRadiusLabel = QtWidgets.QLabel(self.layoutWidget)
        ...
        self.mainImageLabel = QtWidgets.QLabel(self.layoutWidget1)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.vtiSelectLabel.setToolTip(_translate("MainWindow", "<html><head/><body><p>This control provides a drop-down list of VTI (video time inserter) models for which OCR (optical character recognition) timestamp processing is available. Selecting an entry will cause a constellation of character extraction boxes to be displayed on the image and there will be an automatic switch to field view and to a frame other than frame 0 --- which is often an improperly formatted frame with duplicates fields.</p><p><span style=\" font-weight:600; color:#fc0107;\">Note that this control will only be active when an AVI-WCS folder is in use.</span></p><p>If the character selection boxes enclose the correct characters and are well-aligned, then you\'re ready to go.</p><p>If you\'re facing a Kiwi timestamp, which has two positions for the timestamp and switches from one to the other when the mintes field changes, you should first seltect either \'<span style=\" color:#fc0107;\">Kiwi (left)</span>\' or \'<span style=\" color:#fc0107;\">Kiwi (right)</span>\' and keep whichever best encloses the timestamp characters.</p><p>If the character boxes are not well aligned, right-click on the/an offending ocr selection box and use the resulting pop-up menu to decide whether one or all ocr boxes need to be moved. The arrow keys control the jogs and their direction.</p></body></html>"))
