...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.vtiSelectComboBox.setToolTip(_translate("MainWindow", "<html><head/><body><p><span style=\" font-weight:600; color:#fc0107;\">Note: this control will only be active when an AVI-WCS folder is in use.</span></p><p>This control provides a drop-down list of VTI (video time inserter) models for which OCR (optical character recognition) timestamp processing is available. Selecting an entry will cause a constellation of character extraction boxes to be displayed on the image (which automatically switches to field view).</p><p>If the character selection boxes enclose the correct characters and are well-aligned, then you\'re ready to go.</p><p>If you\'re dealing with a Kiwi timestamp, which has two positions for the timestamp character --- a left position and a right position --- you should first select the position that best encloses the timestamp characters in the displayed frame.</p><p>If the character boxes are not well aligned, use the jogging buttons to the right to achieve a better alignment.</p></body></html>"))
